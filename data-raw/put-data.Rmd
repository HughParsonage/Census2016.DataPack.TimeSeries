...
decode_country <- function(out) {
  ...
  out[, CountryOfBirth := gsub("(?<!(F))Ye?a?r.*arrival.ns$", "", CountryOfBirth,
                               ignore.case = TRUE)]
  out[CountryOfBirth %pin% "(Born )?[Ee]lsewhere", CountryOfBirth := NA_character_]
  out[CountryOfBirth == "Country of birth not stated", CountryOfBirth := NA_character_]
  out[CountryOfBirth %pin% "Bosnia", CountryOfBirth := "Bosnia and Herzegovina"]
  out[CountryOfBirth %pin% "Korea", CountryOfBirth := "KOR"]
  out[CountryOfBirth %pin% "Macedonia", CountryOfBirth := "Macedonia"]
  out[CountryOfBirth %pin% c("United Kingdom",
                             "N[a-z]+n Ireland",
                             "England",
                             "Scotland",
                             "Wales"),
  ...
  out[CountryOfBirth %pin% "China excl? SARs (and )?Tai?w",  CountryOfBirth := "CHN"]
...
Mop <- function(DT, value.name = "persons", suborder = NULL) {
  ...
  stopifnot(is.null(varIused) || !any(varIBeingUsed %in% varIused))
  drop_col(DT, "varI")
  ...
  if ("FamilyComposition" %chin% names(out)) {
    if (!OR("CoupleFamily" %in% names(out),
      ...
      # switch to the position of FamilyComposition
      set_colsuborder(out, c("CoupleFamily", "FamilyComposition"))
  ...
  if ("Aboriginal" %chin% names(out) &&
      "TorresStraitIslander" %chin% names(out)) {
    c("Aboriginal", "Both Aboriginal & Torres Strait Islander", "Non-indigenous", 
"Torres Strait Islander")
    out[nor(Aboriginal, TorresStraitIslander), IndigenousStatus := "Non-indigenous"]
    out[`&`(Aboriginal, TorresStraitIslander), IndigenousStatus := "Both Aboriginal & Torres Strait Islander"]
    out[`&`(!Aboriginal, TorresStraitIslander), IndigenousStatus := "Torres Strait Islander"]
    out[`&`(Aboriginal, !TorresStraitIslander), IndigenousStatus := "Aboriginal"]
    out[, c("Aboriginal", "TorresStraitIslander") := NULL]
  }
  
  if ("Age.int" %chin% names(out)) {
    setnames(out, "Age.int", "Age.min")
  }
  ...
  if (anyDuplicated(out, by = setdiff(names(out), value.name))) {
    ...
    stop("2. Duplicated rows.")
  }
  if (!is.null(suborder)) {
    set_colsuborder(out, suborder)
  }
  
  set_cols_last(out, value.name)
...
Mop_multiple <- function(DT) {
  ...
  for (nom in DTnoms) {
    if (!(nom %chin% c(Region_key, "Year"))) {
      ...
      assign(object_name,
             value = decodeRegion(DT[, .SD, .SDcols = c(Region_key, "Year", nom_)]),
             envir = .GlobalEnv)
    }
  }
}
...
freadT(8, c(3142, 3483)) %>%
  .[variable %pin% "Persons$"] %>%
  .[variable %npin% "Total"] %>%
  .[, variable := sub("_20(06|11|16)_Census_", "_", variable)] %>%
  .[, variable := sub("_Persons", "", variable)] %>%
  setnames("variable", "CountryOfBirth") %>%
  decode_country %>%
  .[, .(persons = sum(persons),
        varI = first(varI)), keyby = c(Region_key, "Year", "CountryOfBirth")] %>%
  Mop
```
...
freadT(8, c(3142, 3483)) %>%
  ...
  .[, variable := sub("_20(06|11|16)_Census_", "_", variable)] %>%
  extract_Sex %>%
  .[, variable := sub("_(Males|Females)$", "", variable)] %>%
  setnames("variable", "CountryOfBirth") %>%
  decode_country %>%
  .[, .(persons = sum(persons),
        varI = first(varI)), keyby = c(Region_key, "Year", "CountryOfBirth", "Sex")] %>%
  Mop
```
...
freadT(13, value.name = "students") %>%
  ...
  .[variable %pin% "^Oth",           EduInstitutionType := NA_character_] %>%
    .[, EduInstitutionType := factor(EduInstitutionType,
                                   ...
                                   ordered = TRUE)] %>%
  .[, .(students = sum(students)), keyby = c(Region_key, "Year", "EduInstitutionType")] %>%
  Mop("students")
...
freadT(13, value.name = "students") %>%
  ...
  .[, EduInstitutionType := factor(EduInstitutionType,
                                   ...
                                   ordered = TRUE)] %>%
  extract_Sex %>%
  .[, .(students = sum(students)), keyby = c(Region_key, "Year", "EduInstitutionType", "Sex")] %>%
  Mop("students")
```
...
freadT(19, value.name = "occupied_private_dwellings_being_rented") %>%
  ...
  .[variable %pin% "^20.._Census_([0-9]+)_.*$",
    Rent.min := 52.5 * as.integer(sub("^20.._Census_([0-9]+)_.*$", "\\1", variable))] %>%
  .[, Rent.min := as.integer(Rent.min)] %>%
  .[] %>%
...
freadT(24, value.name = "occupied_private_dwellings_being_rented") %>%
  ...
  .[variable %pin% "^20.._Census_.*_Rent_[0-9]",
    Rent.min := 52.5 * as.integer(sub("^20.._Census_.*_Rent_([0-9]+).*$", "\\1", variable))] %>%
  .[, Rent.min := as.integer(Rent.min)] %>%
  Mop("occupied_private_dwellings_being_rented")
```
...
fread29 %>%
  ...
  
  {
    ...
    for (nom in setdiff(names(dot), c(Region_key, "Year", "Sex"))) {
      ...
      Mop(out, value.name = nom)
      # assign(paste0(Region, "__Year_Sex_", nom),
...
suppressMessages({library(Census2016.spec)})
test_check(data_list = mget(ced_tbls), show.progress = FALSE)

data_path <- "data"
provide.dir("tsv")
provide.dir(file.path("tsv", Region))
tsv_path <- file.path("tsv", Region)
...
for (tbl in ced_tbls) {
  ...
  .ced_tbl <- get(tbl)
  fwrite(.ced_tbl, file = file.tsv)
  
  switch (ceiling(file.size(file.tsv) / (95 * 1024^2)),
          {
            NULL
          },
          {
            ...
            fwrite(DT1, sub("\\.tsv$", "1.tsv", file.tsv))
            fwrite(DT2, sub("\\.tsv$", "2.tsv", file.tsv))
          },
          {
            DT <- .ced_tbl
            lastv <- .subset2(DT, last(names(DT)))
            ...
            file.remove(file.tsv)
            fwrite(DT11, sub("\\.tsv$", "11.tsv", file.tsv))
            fwrite(DT12, sub("\\.tsv$", "12.tsv", file.tsv))
            fwrite(DT21, sub("\\.tsv$", "21.tsv", file.tsv))
            fwrite(DT22, sub("\\.tsv$", "22.tsv", file.tsv))
          })
}
